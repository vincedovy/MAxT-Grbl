                    ...
                    cprequire(
                        ...
                        function(gcodelist) {
                            ...
                            var newOnFileLoaded = function(txt, info, skipLocalStore) {
                                ...
                                var u = txt.match(/(G20|G21)/i);
                                if (gcodelist.isDirtyUnits === false && u === null) {
                                ...
                                if (gcodelist.isDirtyUnits === true) {
                                    isDirty = true;
                                    gcodelist.isDirtyUnits = false;
                                }
                                ...
                                gcodelist.fileLines = txt.split(/\r{0,1}\n/);
                                //gcodelist.fileLines = txt.split(/\n/);

                                // see if last line is empty and if so, drop it
                                if (gcodelist.fileLines[gcodelist.fileLines.length - 1].length == 0) {
                                    gcodelist.fileLines.pop();
                                }
                                ...
                                var newFileLines = [];
                                for (var idx = 0; idx < gcodelist.fileLines.length; idx++) {
                                    var fileLine = gcodelist.fileLines[idx];
                                    if (fileLine.match(/^\s*[%!~]/)) {
                                        ...
                                        gcodelist.fileLines[idx] = fileLine;
                                    }
                                    ...
                                    // remove line numbers
                                    if (gcodelist.options.addlinenums) {
                                    ...
                                    if (gcodelist.options.removeemptylines) {
                                        if (fileLine.match(/^\s*$/)) {
                                ...
                                gcodelist.fileLines = newFileLines;
                                ...
                                if (gcodelist.options.addlinenums) {
                                    for (var idx = 0; idx < gcodelist.fileLines.length; idx++) {
                                        /*if (gcodelist.fileLines[idx].match(/^;|^\(.*\)$/)) {
                                            // don't add line num
                                            //console.warn("we have whole line comment, ignore");
                                            gcodelist.metaLines[idx] = { isComment: true }
                                        } else {*/
                                        gcodelist.fileLines[idx] = "N" + (idx + 1) + " " + gcodelist.fileLines[idx];
                                        //}
                                    }
                                }
                                /*else {
                                ...
                                //gcodelist.metaLines = [gcodelist.fileLines.length];


                                // test code where we set some different statuses
                                /*
                                gcodelist.metaLines[0] = { isQueued: true, isWritten: true, isCompleted: true };
                                gcodelist.metaLines[1] = { isQueued: true, isWritten: true };
                                gcodelist.metaLines[2] = { isQueued: true };
                                */

                                // update dom to show lines
                                $('#com-chilipeppr-widget-gcode-footer #gcode-lines').text(gcodelist.fileLines.length);
                                $('#com-chilipeppr-widget-gcode-footer #gcode-lines-sent').text("0");
                                $('#com-chilipeppr-widget-gcode-footer #gcode-lines-remaining').text(gcodelist.fileLines.length);

                                gcodelist.appendRows(1);

                                //gcodelist.setupInfiniteScroll();
                                gcodelist.setupJumpScroll();

                                // now format the gcode
                                console.log("going to parse gcode to hilite it.");
                                gcodelist.parseGcode();
                                if (skipLocalStore != undefined && skipLocalStore == true) {
                                    ...
                                    try {
                                        ...
                                        if (e.name === 'QUOTA_EXCEEDED_ERR' || e.name == "QuotaExceededError" || e.code == 22 || e.name == "NS_ERROR_DOM_QUOTA_REACHED" || e.code == 1014) {
                                            //gcodelist.sceneRemove(gcodelist.object);
                                            // show err dialog
